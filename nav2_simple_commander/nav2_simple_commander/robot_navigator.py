...
class BasicNavigator(Node):
    def __init__(self, node_name="basic_navigator", namespace=""):
        ...
        self.status = None
        self.namespace = ""
        self.namespace_without_end_slash = ""
        if namespace != "":
            self.namespace = "/" + namespace + "/"
            self.namespace_without_end_slash = "/" + namespace
        ...
        self.nav_through_poses_client = ActionClient(
            self, NavigateThroughPoses, self.namespace + "navigate_through_poses"
        )
        self.nav_to_pose_client = ActionClient(
            self, NavigateToPose, self.namespace + "navigate_to_pose"
        )
        self.follow_waypoints_client = ActionClient(
            self, FollowWaypoints, self.namespace + "follow_waypoints"
        )
        self.follow_path_client = ActionClient(
            self, FollowPath, self.namespace + "follow_path"
        )
        self.compute_path_to_pose_client = ActionClient(
            self, ComputePathToPose, self.namespace + "compute_path_to_pose"
        )
        ...
        self.smoother_client = ActionClient(
            self, SmoothPath, self.namespace + "smooth_path"
        )
        self.spin_client = ActionClient(self, Spin, "spin")
        self.backup_client = ActionClient(self, BackUp, "backup")
        self.assisted_teleop_client = ActionClient(
            self, AssistedTeleop, self.namespace + "assisted_teleop"
        )
        ...
        self.initial_pose_pub = self.create_publisher(
            PoseWithCovarianceStamped, self.namespace + "initialpose", 10
        )
        self.change_maps_srv = self.create_client(
            LoadMap, self.namespace_without_end_slash + "/map_server/load_map"
        )
        ...
        self.get_costmap_global_srv = self.create_client(
            GetCostmap, self.namespace_without_end_slash + "/global_costmap/get_costmap"
        )
        self.get_costmap_local_srv = self.create_client(
            GetCostmap, self.namespace_without_end_slash + "/local_costmap/get_costmap"
        )

        # TODO Remove this -------------------------------------------
        self.info(self.namespace + "navigate_through_poses")
        self.info(self.namespace + "navigate_to_pose")
        self.info(self.namespace + "initialpose")
        self.info(self.namespace_without_end_slash + "/local_costmap/get_costmap")
    ...
    def waitUntilNav2Active(self, navigator="bt_navigator", localizer="amcl"):
        ...
        # self.info("""Block until the full navigation system is up and running.""")
        self._waitForNodeToActivate(localizer)
        if localizer == "amcl":
            self.info("waiting inside amcl check")
    ...
    def _waitForNodeToActivate(self, node_name):
        ...
        self.debug(f"Waiting for {self.namespace}{node_name} to become active..")
        node_service = self.namespace + f"{node_name}/get_state"
